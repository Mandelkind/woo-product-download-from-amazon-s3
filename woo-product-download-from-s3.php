<?php
	/**
	 * Plugin Name:  Woo Product Download from S3
	 * Plugin URI:   https://wordpress.org/plugins/woo-product-download-from-s3/
	 * Description:  WooCommerce Product Download / Upload to / from using S3 service.
	 * Version:      1.1.0
	 * Author:       MANDELKIND
	 * Author URI:   https://mandelkind.swiss/
	 * License:      GPLv2.0+
	 * License URI:  http://www.gnu.org/licenses/gpl-2.0.txt
	 *
	 * Text Domain:  woo-product-download-from-s3
	...
	if ( ! class_exists( 'EA_WC_Amazon_S3' ) ):
		...
		class EA_WC_Amazon_S3 {
			...
			private function constants() {
				...
				$this->ssl_verify_enabled = ( get_option( 'ea_wc_amazon_s3_disable_ssl_verify', 'no' ) === 'yes' ) ? FALSE : TRUE;
			}


			private function endpoint_as_url( $endpoint, $protocol = 'https://' ){
				$endpoint = trim($endpoint);
				if( strpos($endpoint,'://') === false ){ //set HTTPS as default protocol
			...
			public function init() {
				load_plugin_textdomain( 'woo-product-download-from-s3', FALSE, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
			}
			...
			private function hooks() {
				...
				add_action( 'woocommerce_settings_tabs_woo-product-download-from-s3', array( $this, 'settings_fields' ) );

				// Save Settings Fields
				add_action( 'woocommerce_update_options_woo-product-download-from-s3', array( $this, 'update_settings' ) );
			...
			public function upload_handler() {
				...
				if ( ! isset( $_POST[ '_wpnonce' ] ) || ! wp_verify_nonce( $_POST[ '_wpnonce' ], 'ea_wc_amazon_s3_upload_file' ) ) {
					wp_die( esc_html__( 'Not Verified', 'woo-product-download-from-s3' ), esc_html__( 'Error', 'woo-product-download-from-s3' ), array( 'back_link' => TRUE ) );
				}

				if ( empty( $_FILES[ 'ea_wc_amazon_s3_file' ] ) || empty( $_FILES[ 'ea_wc_amazon_s3_file' ][ 'name' ] ) ) {
					wp_die( esc_html__( 'Please select a file to upload', 'woo-product-download-from-s3' ), esc_html__( 'Error', 'woo-product-download-from-s3' ), array( 'back_link' => TRUE ) );
				}
				...
				if ( $this->upload_file( $file ) ) {
					...
					wp_die( esc_html__( 'Something went wrong during the upload process.', 'woo-product-download-from-s3' ), esc_html__( 'Error', 'woo-product-download-from-s3' ), array( 'back_link' => TRUE ) );
				}
			}

			public function upload_file( $file = array() ) {
				if (!file_exists($file['file']) || !is_file($file['file']) || !is_readable($file['file'])){
					return FALSE;
				}
				$s3 = new Aws\S3\S3Client([
					'version'		=> 'latest',
					'region'		=> 'us-east-1',
					'endpoint'		=> $this->endpoint_as_url($this->endpoint),
					'credentials'	=> [
						'key'		=> $this->access_id,
					...
					'http' => [
						'verify' => $this->ssl_verify_enabled
					]
				]);
			...
			public function get_s3_absolute_path() {
				return trailingslashit( $this->endpoint_as_url( get_option('ea_wc_amazon_s3_endpoint'), is_ssl() ? 'https://' : 'http://' ) );
			}
				...
				if ( is_plugin_active( plugin_basename( __FILE__ ) ) ) {
					$action_links = apply_filters( 'ea_wc_amazon_s3_action_links', array(
						'settings' => sprintf( '<a href="%1$s" title="%2$s">%2$s</a>', esc_url( add_query_arg( array(
							                                                                                       'page' => 'wc-settings',
							                                                                                       'tab'  => 'woo-product-download-from-s3'
						                                                                                       ), admin_url( 'admin.php' ) ) ), esc_attr__( 'S3 Settings', 'woo-product-download-from-s3' ) ),
					) );
			...
			public function admin_enqueue_scripts() {
				wp_enqueue_media();
				wp_enqueue_script( 'woo-product-download-from-s3', untrailingslashit( plugin_dir_url( __FILE__ ) ) . '/js/scripts.js', array( 'jquery' ), FALSE, TRUE );
			}
			...
			public function buckets_iframe_content( $type = 'file', $errors = NULL, $id = NULL ) {
				...
				<div style="margin: 20px 1em 1em; padding-right:20px;" id="media-items">
					<?php
						if ( ! $bucket ) { ?>
							<h3 class="media-title"><?php esc_html_e( 'Select a Bucket', 'woo-product-download-from-s3' ); ?></h3>
							<?php
							...
							if ( is_array( $buckets ) ) {
								...
								echo '<tr>';
								echo '<th><strong>' . esc_html__( 'Bucket name', 'woo-product-download-from-s3' ) . '</strong></th>';
								echo '<th><strong>' . esc_html__( 'Actions', 'woo-product-download-from-s3' ) . '</strong></th>';
								echo '</tr>';
								...
								foreach ( $buckets as $key => $bucket ) {
									...
									echo '<td>' . $bucket . '</td>';
									echo '<td>';
									echo '<a href="' . esc_url( add_query_arg( 'bucket', $bucket ) ) . '">' . esc_html__( 'Browse', 'woo-product-download-from-s3' ) . '</a>';
									echo '</td>';
									echo '</tr>';
								}
								echo '</table>';
							...
							if ( is_array( $files ) ) {
								...
								echo '<p><button class="button-secondary" onclick="history.back();">' . esc_html__( 'Go Back', 'woo-product-download-from-s3' ) . '</button></p>';

								echo '<table class="wp-list-table widefat fixed striped" style="max-height: 500px;overflow-y:scroll;">';

								echo '<tr>';
								echo '<th>' . esc_html__( 'File name', 'woo-product-download-from-s3' ) . '</th>';
								echo '<th>' . esc_html__( 'File size', 'woo-product-download-from-s3' ) . '</th>';
								echo '<th>' . esc_html__( 'Uploaded on', 'woo-product-download-from-s3' ) . '</th>';
								echo '<th>' . esc_html__( 'Actions', 'woo-product-download-from-s3' ) . '</th>';
								echo '</tr>';
								...
								foreach ( $files as $key => $file ) {
									...
									echo '<td>' . human_time_diff( $file[ 'time' ] ) . ' ' . esc_html__( 'ago', 'woo-product-download-from-s3' ) . '</td>';

									echo '<td>';
									echo '<a class="insert-s3 button-secondary" href="#" data-s3="' . esc_attr( $file[ 'name' ] ) . '">' . esc_html__( 'Use File', 'woo-product-download-from-s3' ) . '</a>';
									echo '</td>';
									echo '</tr>';
									$i ++;
								}
							...
							if ( isset( $_GET[ 'p' ] ) && $_GET[ 'p' ] > 1 ) {
								echo '<a class="page-numbers prev button-secondary" href="' . esc_url( remove_query_arg( 'p', $base ) ) . '">' . esc_html__( 'Start Over', 'woo-product-download-from-s3' ) . '</a>';
							}
							if ( $i >= 10 ) {
								echo '<a class="page-numbers next button-secondary" href="' . esc_url( add_query_arg( array(
									                                                                                      'p'     => $page + 1,
									                                                                                      'start' => $last_file
								                                                                                      ), $base ) ) . '">' . esc_html__( 'View More', 'woo-product-download-from-s3' ) . '</a>';
							}
							echo '</div>';
							echo '</div>';
						}
			...
			public function media_tabs( $tabs ) {
				$tabs[ 'ea_wc_amazon_buckets' ] = esc_html__( 'Browse S3 Buckets', 'woo-product-download-from-s3' );
				$tabs[ 'ea_wc_amazon_upload' ]  = esc_html__( 'Upload to S3', 'woo-product-download-from-s3' );
			...
			public function upload_iframe_content( $type = 'file', $errors = NULL, $id = NULL ) {
				...
				<script>
					jQuery(document).ready(function ($) {
						$('.woo-product-download-from-s3-insert').on('click', function (e) {
							e.preventDefault();
				...
				<div class="wrap">
					<form enctype="multipart/form-data" method="post" action="<?php echo esc_url( admin_url() ); ?>">
						<table class="form-table">
							<tbody>
							<tr>
								<th><label for="ea_wc_amazon_s3_bucket"><?php esc_html_e( 'Select a bucket', 'woo-product-download-from-s3' ); ?></label></th>
								<td>
									<select name="ea_wc_amazon_s3_bucket" id="ea_wc_amazon_s3_bucket">
										<option value=""><?php esc_html_e( ' - Select a Bucket - ', 'woo-product-download-from-s3' ); ?></option>
							...
							<tr>
								<th><label for="ea_wc_amazon_s3_file"><?php esc_html_e( 'Choose file to upload', 'woo-product-download-from-s3' ); ?></label></th>
								<td>
							...
							<tr>
								...
								<td>
									<input type="submit" name="ea_wc_amazon_s3_upload_submit" class="button-secondary"
									       value="<?php esc_attr_e( 'Upload to S3', 'woo-product-download-from-s3' ); ?>"/>
								</td>
							</tr>
								...
								<tr>
									<td colspan="2">
										<div class="ea_wc_amazon_s3_errors">
											<p class="ea_wc_amazon_s3_success"><?php _e( 'Success! <a href="#" class="woo-product-download-from-s3-insert">Insert uploaded file</a>.' ) ?></p>
										</div>
									</td>
								</tr>
							<?php endif; ?>
			...
			public function get_s3_url( $filename ) {
				...
				$presignedRequest = $s3->createPresignedRequest($cmd, time() + $aws_file_expire_time);
			...
			public function add_tab( $settings_tabs ) {
				$settings_tabs[ 'woo-product-download-from-s3' ] = esc_html__( 'S3 Settings', 'woo-product-download-from-s3' );
			...
			public function settings_options() {
				...
				$settings[] = array(
					'title' => esc_html__( 'S3 Settings', 'woo-product-download-from-s3' ),
					'type'  => 'title',
					'id'    => 'ea_wc_amazon_s3',
				);

				$settings[] = array(
					'name' => esc_html__( 'S3 Access Key ID', 'woo-product-download-from-s3' ),
					'desc' => esc_html__( 'After logging into your Amazon S3 account, click on "Security Credentials" in the sidebar. Scroll down to "Access Keys (Access Key ID and Secret Access Key)" and you will see your Access Key ID. Copy and paste it here.', 'woo-product-download-from-s3' ),
					'id'   => 'ea_wc_amazon_s3_key',
					'type' => 'text',
				);

				$settings[] = array(
					'name' => esc_html__( 'S3 Secret Key', 'woo-product-download-from-s3' ),
					'desc' => esc_html__( 'In the same Access Credentials area, your "Secret Key" will be hidden. You will need to click the "Show" link to see it. Copy and paste it here.', 'woo-product-download-from-s3' ),
					'id'   => 'ea_wc_amazon_s3_secret_key',
					'type' => 'text',
				);

				$settings[] = array(
					'name' => esc_html__( 'S3 EndPoint', 'woo-product-download-from-s3' ),
...
<li>If you create a client by specifying any other AWS Region, each of these regions maps to the region-specific endpoint: <code>s3-&lt;region&gt;.amazonaws.com</code>. </li>
				...
				$settings[] = array(
					'name'    => esc_html__( 'Disable SSL verification', 'woo-product-download-from-s3' ),
					'desc'    => __( 'Disable SSL verification if you run into connection problems.' ),
					'id'      => 'ea_wc_amazon_s3_disable_ssl_verify',
					'type'    => 'checkbox',
					'default' => 'no'
				);
				...
				$settings[] = array(
					'name'    => esc_html__( 'Enable Log', 'woo-product-download-from-s3' ),
